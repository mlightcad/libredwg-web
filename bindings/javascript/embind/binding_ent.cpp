...
emscripten::val dwg_entity_polyline_3d_get_numpoints_wrapper(Dwg_Object_Ptr obj_ptr) {
  Dwg_Object* obj = reinterpret_cast<Dwg_Object*>(obj_ptr);
  int error = 0;
  auto numpoints = dwg_object_polyline_3d_get_numpoints(obj, &error);
...
emscripten::val dwg_entity_polyline_3d_get_points_wrapper(Dwg_Object_Ptr obj_ptr) {
  Dwg_Object* obj = reinterpret_cast<Dwg_Object*>(obj_ptr);
  int error = 0;
  auto numpoints = dwg_object_polyline_3d_get_numpoints(obj, &error);
  if (error != 0) {
  ...
  auto points = dwg_object_polyline_3d_get_points(obj, &error);
  if (error != 0) {
    emscripten::val result = emscripten::val::object();
    result.set("success", false);
    result.set("message", std::string("Failed to get points!"));
    emscripten::val points_obj = emscripten::val::array();
    for (int i = 0; i < numpoints; ++i) {
        points_obj.call<void>("push", point3d_to_js_object(&points[i]));
    }
    result.set("data", points_obj);
    return result;
  }
...
emscripten::val vertex3d_to_js_object(Dwg_Entity_VERTEX_3D* vertex) {
  emscripten::val vertex_obj = emscripten::val::object();
  vertex_obj.set("flag", vertex->flag);
  vertex_obj.set("point", point3d_to_js_object(reinterpret_cast<dwg_point_3d*>(&(vertex->point))));
  return vertex_obj;
...
emscripten::val dwg_entity_polyline_3d_get_vertices_wrapper(Dwg_Object_Ptr obj_ptr) {
  Dwg_Object* obj = reinterpret_cast<Dwg_Object*>(obj_ptr);
  emscripten::val result = emscripten::val::object();
  ...
  if (obj && obj->fixedtype == DWG_TYPE_POLYLINE_3D) {
    ...
    if (dwg->header.version >= R_2004) {
      for (i = 0; i < num_points; i++) {
        ...
        if (vobj && vobj->fixedtype == DWG_TYPE_VERTEX_3D
          ...
          emscripten::val vertex_obj = vertex3d_to_js_object(vertex);
          vertices_obj.call<void>("push", vertex_obj);
        } else {
          result.set("success", false);
          result.set("message", std::string("Not all of vertices returned!"));
    ...
    result.set("success", false);
    result.set("message", "Null pointer passed or object isn't a 3d poyline!");
  }
  return result;
}
...
EMSCRIPTEN_BINDINGS(libredwg_dwg_entity) {
  ...
  DEFINE_FUNC(dwg_entity_polyline_3d_get_numpoints);

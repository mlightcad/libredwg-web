...
export class LibreDwgCore {
      ...
      try {
        ...
        try {
          ...
          if (
            ...
            throw new Error(
              'Failed to decode DWG: WASM memory limit exceeded. Large drawings ' +
        ...
        if (result.error & Dwg_Error.OUTOFMEM) {
          ...
          throw new Error(
            'Failed to decode DWG: out of WASM memory. Rebuild with pnpm build:wasm:32 (INITIAL_MEMORY=1GB) or use a smaller file.'
          )
        }
        if (result.error != 0) {
  ...
  convert(data: Dwg_Data_Ptr) {
    ...
    const converter = new LibreDwgConverter(this as unknown as LibreDwgWasm)
